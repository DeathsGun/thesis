...
Der Wasm-Weg erreicht damit auf keinem Kriterium ein besseres Ergebnis und auf mehreren ein
schlechteres. Er ist dominiert und scheidet aus.
...
Netzwerkschnittstelle auf ein vermitteltes Binding abbildet. Ziel ist, dass die bestehenden
Skripte unverändert weiterlaufen.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    box/.style   ={draw, rounded corners=2pt, align=left, inner sep=4pt,
                   font=\scriptsize, text width=5.0cm, fill=white, anchor=center},
    check/.style ={box, fill=black!6},
    side/.style  ={box, text width=5.4cm},
    lbl/.style   ={font=\scriptsize\itshape, inner sep=1.5pt, fill=white},
  ]
    \node[box]   (a) at (0, 0)    {Aufruf trifft ein: Mandant, Inhaltskennung des Bundles,
                                   vorsignierte URL};
    \node[check] (b) at (0,-1.5)  {Ist dem Mandanten bereits ein Pod zugewiesen?};
    \node[check] (c) at (0,-3.9)  {Liegt das Bundle unter dieser Kennung im Zwischenspeicher?};
    \node[box]   (d) at (0,-6.2)  {Isolate aus dem Laufzeit-Snapshot erzeugen, Bindings
                                   registrieren, Mandantenkontext in \texttt{OpState} ablegen};
    \node[box]   (e) at (0,-7.8)  {Bundle auswerten und Einstiegspunkt aufrufen};
    \node[box]   (f) at (0,-9.2)  {Event Loop leerlaufen lassen, danach das Isolate verwerfen.
                                   Der Zwischenspeicher bleibt bestehen.};

    \node[side]  (b2) at (7.1,-1.5) {Pod aus dem vorgehaltenen Pool zuweisen, Prozess starten
                                     und die aktuell aufgerufenen Bundles vorladen};
    \node[side]  (c2) at (7.1,-3.9) {über die vorsignierte URL laden, gegen die Inhaltskennung
                                     prüfen und aufnehmen, sofern die Herkunft cachefähig ist
                                     und das Budget es zulässt};

    \draw[->] (a) -- (b);
    \draw[->] (b) -- node[lbl,pos=0.25]{ja} (c);
    \draw[->] (c) -- node[lbl,pos=0.25]{ja} (d);
    \draw[->] (d) -- (e);
    \draw[->] (e) -- (f);

    \draw[->] (b.east) -- node[lbl,above]{nein} (b2.west);
    \draw[->] (c.east) -- node[lbl,above]{nein} (c2.west);
    \draw     (b2.south) |- (0,-2.75);
    \draw     (c2.south) |- (0,-5.15);
  \end{tikzpicture}
  \caption{Ablauf einer einzelnen Ausführung mit Pod-Zuweisung und Zwischenspeicher}
  \label{fig:ablauf}
\end{figure}

\autoref{fig:ablauf} fasst den Ablauf zusammen. Die Zuteilung sucht den Pod des Mandanten und weist andernfalls
